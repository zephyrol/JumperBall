...
FontTexturesGenerator::FontTexturesGenerator(CstTextureSampler_uptr lettersTexture,
                                             vecTextLabel_sptr&& textLabels)
    : _lettersTexture(std::move(lettersTexture)), _messageLabels(std::move(textLabels)) {}

FontTexturesGenerator::FTContent FontTexturesGenerator::initFreeTypeAndFont(const unsigned char* fontData,
                                                                            size_t fontDataSize) {
    ...
    const auto ftOpenFaceResult = FT_Open_Face(ftContent.ftLib, &openArgs, 0, &ftContent.fontFace);
...
FontTexturesGenerator::GraphicCharacter FontTexturesGenerator::createOrGetGraphicCharacter(
    ...
    FT_UInt pixelHeight,
    LettersTextureState& lettersTexture,
    ...
    const auto pixelsUv = insertCharacterToTexture(lettersTexture, bitmap.buffer, width, rows);

    const FontTexturesGenerator::GraphicCharacter graphicCharacter = {{pixelsUv.x, pixelsUv.y},
...
FontTexturesGenerator_uptr FontTexturesGenerator::createInstance(
    ...
    const FontTexturesGenerator::FTContent& ftContent) {
    FontTexturesGenerator::LettersTextureState lettersTexture;
    ...
    for (auto& textNode : textNodes) {
        ...
        const auto nodePixelHeight =
            static_cast<FT_UInt>(std::ceil(static_cast<float>(screenHeight) * screenSpaceHeight));
        ...
        for (unsigned char c : message) {
            const auto graphicCharacter =
                createOrGetGraphicCharacter(ftContent, nodePixelHeight, lettersTexture, c);
        ...
        const Node_sptr centeredNode = std::make_shared<CenteredNode>(
            node, static_cast<float>(nodePixelWidth) / static_cast<float>(nodePixelHeight));

        auto textLabel = std::make_shared<TextLabel>(
            std::move(textNode), centeredNode, lettersUvs,
            getCharacterLocalTransforms(transforms, nodePixelWidth, nodePixelHeight), nodePixelHeight,
            JBTypes::Color::Blue);
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_R8, static_cast<GLsizei>(lettersTexture.width),
                 static_cast<GLsizei>(lettersTexture.height), 0, GL_RED, GL_UNSIGNED_BYTE,
...
glm::ivec4 FontTexturesGenerator::insertCharacterToTexture(
    ...
    // Update cursor
    lettersTexture.cursor = {drawingCursor.x + extendedWidth, drawingCursor.y};
