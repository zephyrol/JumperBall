...
DepthFrameBuffer::DepthFrameBuffer(CstTextureSampler_uptr renderTexture,
...
DepthFrameBuffer_uptr DepthFrameBuffer::createInstance(GLsizei resolutionX, GLsizei resolutionY) {
    ...
    glTexStorage2D(GL_TEXTURE_2D, 1, GL_DEPTH_COMPONENT16, resolutionX, resolutionY);
    ...
    return DepthFrameBuffer_uptr(new DepthFrameBuffer(std::move(depthTexture), std::move(gpuFrameBuffer)));
}
