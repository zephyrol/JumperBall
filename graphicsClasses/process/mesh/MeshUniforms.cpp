...
MeshUniforms::MeshUniforms(CstDisplayable_sptr displayable,
...
MeshUniforms MeshUniforms::createInstance(const CstDisplayable_sptr& displayable,
                                          const ShaderProgram_uptr& shaderProgram,
                                          short dynamicsId) {
    auto uniformVec4s = createUniformsFromDynamics<UniformVec4>(displayable->getDynamicQuaternionValues(),
                                                                displayable->getDynamicQuaternionNames(),
                                                                dynamicsId, shaderProgram);

    auto uniformVec3s = createUniformsFromDynamics<UniformVec3>(
        displayable->getDynamicVec3fValues(), displayable->getDynamicVec3fNames(), dynamicsId, shaderProgram);

    auto uniformVec2s = createUniformsFromDynamics<UniformVec2>(
        displayable->getDynamicVec2fValues(), displayable->getDynamicVec2fNames(), dynamicsId, shaderProgram);

    auto uniformFloats = createUniformsFromDynamics<UniformFloat>(
        displayable->getDynamicFloatValues(), displayable->getDynamicFloatNames(), dynamicsId, shaderProgram);

    auto uniformInts = createUniformsFromDynamics<UniformInt>(
        displayable->getDynamicIntValues(), displayable->getDynamicIntNames(), dynamicsId, shaderProgram);

    return MeshUniforms(displayable, std::move(uniformVec4s), std::move(uniformVec3s),
                        std::move(uniformVec2s), std::move(uniformFloats), std::move(uniformInts));
}
