...
class MeshUniforms {
   public:
    static MeshUniforms createInstance(const CstDisplayable_sptr& displayable,
                                       const ShaderProgram_uptr& shaderProgram,
                                       short dynamicsId);

    explicit MeshUniforms(CstDisplayable_sptr displayable,
   ...
   private:
    ...
    void updateUniforms(const std::vector<T1>& newValues, T2& currentUniforms);
};
...
std::vector<std::shared_ptr<UniformType>> MeshUniforms::createUniformsFromDynamics(
    ...
    for (size_t i = 0; i < dynamics.size(); ++i) {
        uniforms.emplace_back(
            std::make_shared<UniformType>(Utility::convertToOpenGLFormat(dynamics[i]), shaderProgram,
