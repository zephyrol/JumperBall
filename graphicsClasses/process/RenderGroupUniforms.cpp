...
RenderGroupUniforms::RenderGroupUniforms(
    vecMeshUniforms_sptr &&dynamicUniforms
) :
    _dynamicUniforms(std::move(dynamicUniforms)) {
}

RenderGroupUniforms RenderGroupUniforms::createInstance(
    ...
    for (const auto &meshList: meshDynamicGroup.dynamicMeshes()) {
        ...
        const auto meshUniforms = std::make_shared<MeshUniforms>(
            meshList.front()->genMeshUniforms(shaderProgram)
        );
