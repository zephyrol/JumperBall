...
RenderingCache::RenderingCache(size_t objectsLifetime)
    : _objectsLifetime(objectsLifetime), _frameBuffers{}, _shaderPrograms{}, _textures{} {}
...
CstTextureSampler_uptr RenderingCache::getTexture(const std::string& key) {
    return getRenderingObject(key, _textures);
}
...
void RenderingCache::setTexture(const std::string& key, CstTextureSampler_uptr texture) {
    CachedRenderingObject<CstTextureSampler_uptr> cachedTexture{std::move(texture), _objectsLifetime};
    _textures.emplace(key, std::move(cachedTexture));
}
