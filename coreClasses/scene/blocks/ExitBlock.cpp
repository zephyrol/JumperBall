...
vecCstShape_sptr ExitBlock::getExtraShapes() const {
    ...
    shapes.push_back(std::make_shared<const Shape>(
        Shape::Aspect::Cylinder, _isUnlockModel ? JBTypes::Color::ShinyGreen : JBTypes::Color::Red,
...
void ExitBlock::unlockExit(bool makeSound) {
    _isUnlocked = true;

    if (_isUnlockModel && makeSound) {
...
Block::Effect ExitBlock::detectionEvent() {
    ...
    if (_isUnlocked && ball->currentSide() == _exitDir) {
        ball->addUpdateOutput(std::make_shared<SoundOutput>("hasWon"));
        return Block::Effect::FinishLevel;
    }
