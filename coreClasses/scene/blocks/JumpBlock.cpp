...
JumpBlock::JumpBlock(const JBTypes::vec3ui& position,
    ...
    : InteractiveBlock(position, items, enemies, specials, ball), _facesJumpers(facesJumpers) {}
...
vecCstShape_sptr JumpBlock::getExtraShapes() const {
    ...
    for (size_t i = 0; i < _facesJumpers.size(); i++) {
        ...
        if (isJumper) {
            ...
            const JBTypes::Dir direction = JBTypesMethods::integerAsDirection(static_cast<unsigned int>(i));

            const JBTypes::vec3f localScale{0.7f, 0.05f, 0.7f};

            const JBTypes::vec3f translationOnBlock{0.f, offset, 0.f};

            const JBTypes::vec3f translationPosition{static_cast<float>(_position.at(0)),
            ...
            shapes.push_back(std::make_shared<const Shape>(
                Shape::Aspect::Cylinder, JBTypes::Color::Yellow,
