...
template<typename T> class ParallelTask {
public:
...
template<typename T> ParallelTask <T>::ParallelTask(std::function <T(size_t)>&& taskFunction,
...
template<typename T> void ParallelTask <T>::runTasks() {
    _asyncTasks.clear();
...
template<typename T> const std::shared_ptr <std::vector <T> >& ParallelTask <T>::waitTasks() {
...
template<typename T> void ParallelTask <T>::threadFunction (const std::function <T(size_t)>& task,
                                                            size_t threadnumber,
                                                            size_t nbOfThreads,
                                                            size_t nbOfTasks) {

    for (
        size_t i = (threadnumber * nbOfTasks / nbOfThreads);
...
template<typename T> void ParallelTask <T>::runTask (
    const std::function <T(size_t)>& task, size_t taskNumber) {
    _returnValues->at(taskNumber) = task(taskNumber);
}

template<> void ParallelTask <void>::runTask(const std::function <void(size_t)>& task,
                                             size_t taskNumber);


template<typename T> std::shared_ptr <std::vector <T> > ParallelTask <T>::allocateReturnValues() {
    return std::make_shared <std::vector <T> >(_numberOfTasks);
}

template<> std::shared_ptr <std::vector <void> > ParallelTask <void>::allocateReturnValues();

template<typename T> size_t ParallelTask <T>::getNumberOfThreads (size_t numberOfTasks) {
